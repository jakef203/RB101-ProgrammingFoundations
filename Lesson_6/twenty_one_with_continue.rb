...
def msg_continue(message)
  print "--> #{message}#{(' ' * 10)}(continue)"
  continue_game
  print "\r"
  prompt (message + (' ' * 40))
end
...
prompt "Press any button to begin game or when prompted with '(continue)'."
continue_game
loop do
  ...
  until won_5_games(score)
    ...
    loop do
      ...
      deal_card(card_deck, card_hand, :player)
      msg_continue "Player chose to hit!"
      msg_continue "Player has: #{joinor(card_hand[:player][:cards], ',', 'and')} for a total of #{card_hand[:player][:total]}"
            # "for a total of #{card_hand[:player][:total]}"
    ...
    if busted?(card_hand, :player)
      ...
      msg_continue "Player stays with a #{card_hand[:player][:total]}."
      puts ""
    end
    msg_continue "Dealer has: #{joinor(card_hand[:dealer][:cards], ',', 'and')} " \
          "for a total of #{card_hand[:dealer][:total]}."
    loop do
      break if card_hand[:dealer][:total] >= DEALER_STAY
      msg_continue "Dealer hits!"
      # continue_game
      deal_card(card_deck, card_hand, :dealer)
      puts ""
      break if busted?(card_hand, :dealer)
      msg_continue "Dealer has: #{joinor(card_hand[:dealer][:cards], ',', 'and')} for a total of #{card_hand[:dealer][:total]}."
                  #  "for a total of #{card_hand[:dealer][:total]}."
    end
    ...
    if busted?(card_hand, :dealer)
      ...
      msg_continue "Dealer stays at #{card_hand[:dealer][:total]}."
      puts ""
    end
    display_result(card_hand, score)
    continue_game
